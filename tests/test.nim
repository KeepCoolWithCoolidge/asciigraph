...
suite "asciigraph tests":
  ...
  test "Graph 2":
    let
      data = @[2.0, 1, 1, 2, -2, 5, 7, 11, 3, 7, 1]
      graph = readFile(prefix / "t2.txt")
    check plot(data) == graph

  test "Graph 3":
    let
      data = @[2.0, 1, 1, 2, -2, 5, 7, 11, 3, 7, 4, 5, 6, 9, 4, 0,
                  6, 1, 5, 3, 6, 2]
  ...
  test "Graph 4":
    let
      data = @[0.2, 0.1, 0.2, 2, -0.9, 0.7, 0.91, 0.3, 0.7, 0.4, 0.5]
  ...
  test "Graph 6":
    let
      data = @[0.453, 0.141, 0.951, 0.251, 0.223, 0.581, 0.771, 0.191,
              0.393, 0.617, 0.478]
  ...
  test "Graph 7":
    let
      data = @[0.01, 0.004, 0.003, 0.0042, 0.0083, 0.0033, 0.0079]
      graph = readFile(prefix / "t7.txt")
    check plot(data) == graph

  test "Graph 8":
    let
      data = @[192.0, 431, 112, 449, -122, 375, 782, 123, 911, 1711, 172]
      graph = readFile(prefix / "t8.txt")
    check plot(data, height = 10) == graph

  test "Graph 9":
    let
      data = @[0.3189989805, 0.149949026, 0.30142492354,
  ...
  test "Graph 10":
    let
      data = @[0.0, 0, 0, 0, 1.5, 0, 0, -0.5, 9, -3, 0, 0, 1, 2, 1,
  ...
  test "Graph 11":
    let
      data = @[-5.0, -2, -3, -4, 0, -5, -6, -7, -8, 0, -9, -3, -5,
              -2, -9, -3, -1]
  ...
  test "Graph 12":
    let
      data = @[-0.000018527, -0.021, -0.00123,
  ...
  test "Graph 13":
    let
      data = @[57.76, 54.04, 56.31, 57.02, 59.5, 52.63, 52.97,
